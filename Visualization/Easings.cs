...
public static class Easings
{
    ...
    // Cubic Easing functions
    public static float EaseCubicInClamped(float t, float b, float c, float d)
    {
        var value = (c * (t /= d) * t * t + b);
        var min = Math.Min(c, d);
    ...
    public static float EaseCubicOutClamped(float t, float b, float c, float d)
    {
        var value = (c * ((t = t / d - 1) * t * t + 1) + b);
        var min = Math.Min(c, d);
        var max = Math.Max(c, d);
        return Math.Clamp(value, min, max);
    }
